...
if __name__ == '__main__':
    ...
    #    ax.set_yscale('log', basey=2)
        ...
        for tile_i, tile_x in reverse_tiles:
            ...
            # Filter tiles_x and rates by nonzero
            real_files_c = files_x[real_rates]
            real_files_r = tile_rates[real_rates]

            # Scatter plot prep
            real_dots = tile_dots.T[real_rates].T

            real_err = np.std(real_dots, 0)

            # Plot the rates for this tile shape
            tile_label = '{} pixel tiles'.format(tile_x)
            ax.errorbar(real_files_c, real_files_r, real_err, label=tile_label, marker='o')

       
        # Plot the rates for the maximum tile shapes by file size
        tile_label = 'one tile per file'
        ax.plot(files_x[:-3], ft_rate[:-3], label=tile_label, color='black', ls='--')
    ...
    #    ax.set_yscale('log', basey=2)
        ...
        file_size_count = zip(files_x,files_c)
        plt.xticks(files_x, map(ticker, file_size_count)) 
        #ax.set_xlim(xmax = files_c[-3]) 

        # Label the graph
        plt.ylabel('Speed (MiB per second)')
        plt.xlabel('Stored file size in pixels\n(total number of files)')
        title_fmt = 'Rate to read tile sizes from file sizes'
        # Make a big title
        title_font = dict(fontsize=18)
        ...
        # Plot the rates for the maximum tile shapes by file size
        tile_label = 'one tile per file'
        ax.plot(files_c[:-3], ft_rate[:-3], label=tile_label, color='black', ls='--')
    ...
    #    ax.set_yscale('log', basey=2)
        ...
        title_fmt = 'Rate to read multiple tiles per file'
        # Make a big title
        title_font = dict(fontsize=18)
        plt.title(title_fmt, **title_font)
        # Write to file
        ...
        for tile_i, tile_x in reverse_tiles:
            ...
            # Plot the rates for this tile shape
            tile_label = '{} pixel tiles'.format(tile_x)
            ax.errorbar(real_ratios, real_times, real_err, label=tile_label, marker='o')

        plt.xlim(xmin=1-0.125)
        plt.ylim(0,550)
        # Power of 2 X axis
        ax.set_xscale('log', basex=2)
    #    ax.set_yscale('log', basey=2)
